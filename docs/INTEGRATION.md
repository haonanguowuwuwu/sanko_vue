...
**原则：Mock 代码保留，通过环境变量或 npm 命令切换，无需删除任何 Mock 实现。**

| 项 | 说明 |
|----|------|
| 代码逻辑 | `sanko_read/src/api/config.ts`：`USE_MOCK = (VITE_USE_MOCK !== 'false')` |
| 生效方式 | 各 `sanko_read/src/api/*.ts` 在 Mock 时短路，不调用 `request()` |
| 默认值 | `VITE_USE_MOCK=true`（见 `sanko_read/.env.development`、`.env.example`） |
| 小后端地址 | `VITE_API_BASE_URL`，推荐 `http://127.0.0.1:8083`（Windows 联调更稳） |

#### 方式 A：npm 命令切换（推荐）

| 模式 | 命令 | 读取的配置 | 行为 |
|------|------|------------|------|
| 内置 Mock | `npm run dev:mock` | `.env.mock` | 不请求小后端 |
| 联调小后端 | `npm run dev:api` | `.env.api` | 请求 `VITE_API_BASE_URL` |
| 默认 dev | `npm run dev` | `.env.development` + 可选 `.env.development.local` | 由本地覆盖文件决定 |

对应环境文件（均在 `sanko_read/` 下）：

| 文件 | 用途 |
|------|------|
| `.env.mock` | `dev:mock` 专用，`VITE_USE_MOCK=true` |
| `.env.api` | `dev:api` 专用，`VITE_USE_MOCK=false` |
| `.env.development` | 仓库默认，`VITE_USE_MOCK=true` |
| `.env.development.local` | 本地覆盖（git 忽略 `*.local`），可复制 `.env.development.local.example` |
| `.env.example` | 模板与说明 |
...
之后 `npm run dev` 即为联调模式。

#### 如何确认当前模式

| 位置 | 表现 |
|------|------|
| 终端（Vite 启动时） | 打印 `内置 Mock` 或 `联调小后端 http://127.0.0.1:8083` |
| 浏览器控制台 | `[Sanko] 内置 Mock（未请求小后端）` 或 `[Sanko] 联调小后端 …` |
| Network | Mock 时无 `/api/*`；联调时有对 8083 的请求 |

> **切换模式后请重新登录**：Mock 的 `mock-token-…` 与小后端的 `token-…` 不通用。
...
### 3.1 推荐：`npm run dev:api`

```bash
cd sanko_server && npm run dev    # 终端 1，8083
cd sanko_read && npm run dev:api    # 终端 2，5173
```
...
然后 `npm run dev`。修改任何 `.env*` 后 **必须重启** dev server。

### 3.3 环境变量说明

| 变量 | 说明 |
|------|------|
| `VITE_API_BASE_URL` | 小后端 Base URL，不要末尾斜杠；推荐 `http://127.0.0.1:8083` |
| `VITE_USE_MOCK` | 仅字符串 `false` 时关闭 Mock；`true` 或未设置均为 Mock |

### 3.4 验证 Mock 已关闭

1. 终端启动日志应显示「联调小后端」
2. 浏览器控制台应有 `[Sanko] 联调小后端 …`
3. DevTools → **Network** 刷新或登录，应看到 `http://127.0.0.1:8083/api/...`

若仍无请求，检查：

- 是否用了 `npm run dev:mock` 或 `.env.development.local` 里仍是 `true`
- 是否已重启 dev server
- 小后端是否已启动（`GET http://127.0.0.1:8083/health`）
...
或使用 `.env.development.local` + `npm run dev`（见第 3 节）。
...
- 确认使用 `npm run dev:api`，或 `.env.development.local` 中 `VITE_USE_MOCK=false`
- 确认已重启 dev server；看终端与控制台是否仍为「内置 Mock」
- `dev:mock` 与 `dev:api` 使用独立 mode，**不会**读取对方的 `.env.development.local`

### Q2：登录报 CORS 错误

小后端已允许 `http://localhost:5173` 与 `http://127.0.0.1:5173`。若仍报错，检查 `VITE_API_BASE_URL` 与浏览器地址栏主机名是否一致（建议统一用 `127.0.0.1`）。
...
- [ ] 联调使用 `npm run dev:api`（或 `.env.development.local` 中 `VITE_USE_MOCK=false`）
- [ ] `VITE_API_BASE_URL` 指向 `http://127.0.0.1:8083`
- [ ] 已重启前端；终端 / 控制台显示「联调小后端」
- [ ] Network 中能看到 `/api/*` 请求
- [ ] 切换模式后已重新登录
...
| Vite 启动提示 | `sanko_read/vite.config.ts`（`sanko-dev-banner` 插件） |
| npm 脚本 | `sanko_read/package.json`：`dev:mock`、`dev:api` |
| 环境文件 | `sanko_read/.env.mock`、`.env.api`、`.env.development.local.example` |
